...
export default function ResumePage() {
  ...
  const [range, setRange] = useState<"3m" | "6m" | "custom">("6m");
  ...
  useEffect(() => {
    posthog.capture("generator_started", { type: "resume" });
    loadEntryCount();
  }, []);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(error.error || "Failed to generate resume bullets");
      }

      const data = await res.json();
      setOutput(data.output_markdown);
      posthog.capture("generator_succeeded", { type: "resume", entry_count: data.entry_count });
      toast.success("Resume bullets generated!");
    } catch (error) {
      console.error("Generate error:", error);
      posthog.capture("generator_failed", { type: "resume", error: String(error) });
      toast.error(error instanceof Error ? error.message : "Failed to generate resume bullets");
    } finally {
  ...
  const handleCopy = () => {
    if (output) {
      ...
      posthog.capture("generator_output_copied", { type: "resume" });
  ...
  const handleDownload = () => {
    if (output) {
      ...
      a.href = url;
      a.download = `resume-bullets-${format(new Date(), "yyyy-MM-dd")}.md`;
  ...
  if (entryCount < 5) {
    return (
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Resume Bullets Generator</h1>
          <p className="text-muted-foreground mt-1">
            Generate punchy resume bullets from your accomplishments
          </p>
        </div>
        ...
        <Card className="shadow-md border-border">
          <CardContent className="p-8 text-center">
            ...
            <p className="text-muted-foreground mb-4">
              You need at least 5 entries to generate resume bullets. You currently have{" "}
              {entryCount}.
            </p>
            <Button
    ...
    <div className="space-y-6">
      ...
      <Card className="shadow-md border-border">
        <CardHeader>
          ...
          <CardDescription>Choose the period to include in your resume bullets</CardDescription>
        </CardHeader>
